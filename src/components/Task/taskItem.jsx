...
export default function TaskItem({ task, i, deleted = false }) {
  ...
  return (
    <div className="w-full m-2 my-4 grid gap-x-4 grid-cols-12 items-center ">
      ...
      <div className="col-start-1 col-span-4 md:col-span-2 lg:col-span-3 py-2 flex md:justify-end">
        ...
        <span
          className="py-2 rounded-full flex items-center justify-center text-xs w-28 min-w-fit"
          style={style}
        >
          {task.status}
        </span>
      ...
      {popup && (
        <AddTaskPopup showPopup={setPopup} taskEdit={task} editMod={true} />
      )}
    </div>
  );
}
